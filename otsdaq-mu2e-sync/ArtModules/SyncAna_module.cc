...
namespace ots
{
  ...
  public:
    struct Config
    {
      ...
      fhicl::Atom<std::string> mtpTag{Name("mtpTag")   , Comment("Timing paddle hit collection tag")};
  ...
  private:
    ...
    // Data
    const MTPHitCollection*               mtpHits_;
...
void ots::SyncAna::bookHistograms(const int index) {
  if(index >= kMaxHists) throw cet::exception("BADCONFIG") << "Histogram index " << index << " larger than maximum " << kMaxHists;
  if(hist_[index]) throw cet::exception("BADCONFIG") << "Histogram index " << index << " already booked!";
...
void ots::SyncAna::fillHistograms(const int index) {
  if(!hist_[index]) throw cet::exception("BADCONFIG") << "Histogram index " << index << " not booked!";
  Hist_t* Hist = hist_[index];
  Hist->nhits  ->Fill(mtpHits_->size());
...
void ots::SyncAna::analyze(art::Event const& event)
{
  ...
  art::Handle<mu2e::MTPHitCollection> mtpH;
  if(!event.getByLabel(mtpTag_, mtpH) || !mtpH.product()) {
    TLOG(TLVL_WARNING) << "[SyncAna::" << __func__ << "] No MTP hit collection found!\n";
    mtpHits_ = nullptr;
  } else {
    mtpHits_ = mtpH.product();
    TLOG(TLVL_DEBUG + 21) << "[SyncAna::" << __func__ << "] Event " << event.id()
                          << " has " << mtpHits_->size() << " MTP hits\n";
  }

  // fill the histograms of interest
  if(mtpHits_) fillHistograms(0);

  if(!mtpHits_->empty()) { // new hit found
    prev_hit_time_ = mtpHits_->back().time();
    evt_since_last_hit_ = 1;
  } else if(evt_since_last_hit_ >= 0) ++evt_since_last_hit_; // increment if no hit was found
