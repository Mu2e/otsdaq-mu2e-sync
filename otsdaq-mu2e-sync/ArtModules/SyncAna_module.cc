...
void ots::SyncAna::beginJob() {
  ...
  for (int index = 0; index < kMaxHists; ++index)
    hist_[index] = nullptr;
...
void ots::SyncAna::bookHistograms(const int index) {
  ...
  if (hist_[index])
    throw cet::exception("BADCONFIG") << "Histogram index " << index << " already booked!";
...
void ots::SyncAna::fillHistograms(const int index) {
  if (!hist_[index])
    throw cet::exception("BADCONFIG") << "Histogram index " << index << " not booked!";
  Hist_t* Hist = hist_[index];
...
void ots::SyncAna::analyze(art::Event const& event) {
  ++evtCounter_;
  TLOG(TLVL_DEBUG + 20) << "[SyncAna::" << __func__ << "] Analyzing event " << evtCounter_ << ": "
                        << event.id() << std::endl;

  art::Handle<mu2e::MSDHitCollection> msdH;
  if (!event.getByLabel(msdTag_, msdH) || !msdH.product()) {
    TLOG(TLVL_WARNING) << "[SyncAna::" << __func__ << "] No MSD hit collection found!\n";
    msdHits_ = nullptr;
  } else {
    msdHits_ = msdH.product();
    TLOG(TLVL_DEBUG + 21) << "[SyncAna::" << __func__ << "] Event " << event.id() << " has "
                          << msdHits_->size() << " MSD hits\n";
  }
...
void ots::SyncAna::endJob() {
  __COUT__ << "[SyncAna::" << __func__ << "] Ending job, saw " << evtCounter_ << " events\n";
}
